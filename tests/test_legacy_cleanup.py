...
def test_install_factory_removes_legacy_structure_after_spinning_down(tmp_path: Path):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        with patch.object(install_factory, "validate_target_repo"), \
             patch.object(install_factory, "clone_factory"), \
             patch.object(install_factory, "update_factory", return_value="v1.0"), \
             patch.object(install_factory, "head_commit", return_value="abcdef"), \
             patch.object(install_factory, "run_command"), \
             patch.object(install_factory, "invoke_bootstrap"):
