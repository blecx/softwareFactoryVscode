...
## Local parity surfaces: the four-level mirrored validation contract

Use the canonical local parity entrypoints governed by the shared validation rule engine. All execution uses the same bundle composition as GitHub CI, follows local-first then GitHub-confirmed explicit exception semantics, and is subject to the **bounded-runtime/watchdog rule** (45-minute strict cap per bundle).

- `./.venv/bin/python ./scripts/local_ci_parity.py --level focused-local` is the fastest subset for local loop iteration.
- `./.venv/bin/python ./scripts/local_ci_parity.py --level pr-update` provides PR-incremental scope.
- `./.venv/bin/python ./scripts/local_ci_parity.py --level merge` provides canonical PR-readiness evidence for issue handoff.
...
- `./.venv/bin/python ./scripts/local_ci_parity.py --level production --fresh-checkout` is the closest local replay of GitHub's checkout-and-bootstrap behavior. It creates a clean git worktree snapshot, runs `./setup.sh`, and then replays the canonical production gate there.

GitHub Checks now mirrors this exact structure via explicit CI jobs generated by the shared engine. The aggregate job remains the readiness contract authority, keeping required-check continuity deterministic and preventing blind exception skip drift.
...
- the repo CI-parity baseline via `./.venv/bin/python ./scripts/local_ci_parity.py --level merge`;
- the canonical blocking production parity command via `./.venv/bin/python ./scripts/local_ci_parity.py --level production`;
- the exact fresh-checkout replay path via `./.venv/bin/python ./scripts/local_ci_parity.py --level production --fresh-checkout` whenever merge-grade confidence depends on GitHub-like checkout/bootstrap semantics;
- runtime verification against the generated effective endpoints and manager-backed readiness surface;
- the blocking Docker E2E runtime proof lane, currently satisfied by the promoted strict-tenant, stop/cleanup, and backup/restore roundtrip scenarios within `./.venv/bin/python ./scripts/local_ci_parity.py --level production`;
