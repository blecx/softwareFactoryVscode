...
For PR-error repair, those canonical sources now share the same fast evidence-first default captured by `pr-error-resolve-tactic.prompt.md`.
...
- [`../../.github/prompts/pr-error-resolve-tactic.prompt.md`](../../.github/prompts/pr-error-resolve-tactic.prompt.md) — canonical focused entrypoint for the repository's default fast evidence-first PR-error repair method: parse exact output, start with the cheapest failing gate, and widen validation only after the narrow gate passes.
- [`../../.github/prompts/apply-canonical-validation-policy.prompt.md`](../../.github/prompts/apply-canonical-validation-policy.prompt.md) — canonical surface that reads configs/validation_policy.yml without declaring validation rules itself.
...
- [`execute-github-issues-in-order.prompt.md`](../../.github/prompts/execute-github-issues-in-order.prompt.md) — manual ordered queue execution with explicit stop gates, GitHub-truth rechecks, and one-issue-at-a-time discipline. It routes into the `workflow` agent and then back into the same canonical `resolve-issue` → `pr-merge` slice path. It is not a second implementation workflow. Use [`@execute-approved-plan`](../../.github/agents/execute-approved-plan.md) instead when the operator has already approved a bounded issue set that may continue automatically.
- [`execute-approved-plan.prompt.md`](../../.github/prompts/execute-approved-plan.prompt.md) — dedicated activation surface for alias-heavy approved-plan requests such as `execute the plan` or `run the approved queue`. It routes directly into the specialized [`@execute-approved-plan`](../../.github/agents/execute-approved-plan.md) agent so bounded approved-plan execution does not drift into generic planning behavior. It still reuses the same canonical `resolve-issue` → `pr-merge` slice path.
- [`resume-after-interruption.prompt.md`](../../.github/prompts/resume-after-interruption.prompt.md) — deterministic recovery after restart, compaction, timeout, or operator handoff. It re-anchors from `.tmp/github-issue-queue-state.md`, writes `.tmp/interruption-recovery-snapshot.md`, and resumes the currently active workflow instead of inventing a new one.
- [`apply-canonical-validation-policy.prompt.md`](../../.github/prompts/apply-canonical-validation-policy.prompt.md) — ensures validation bounds and logic respect `configs/validation_policy.yml`. It routes into the `workflow` agent and delegates the decision of what to run to the central policy file.
- [`pr-error-resolve-tactic.prompt.md`](../../.github/prompts/pr-error-resolve-tactic.prompt.md) — fast PR/CI failure resolution using exact output parsing, narrow reproductions, and root-cause-first repair sequencing. It routes into the `workflow` agent as a focused activation surface for the repository's default PR-error repair method. It does not replace the canonical `resolve-issue` → `pr-merge` workflow; it makes the default repair behavior explicit: start with the cheapest failing gate, state the exact failure evidence, avoid guessing or hallucinated state, and widen validation only after the fast gate passes.
...
## AI surface form note

- This page is a plain-markdown maintainer index; it does not define runtime discovery rules.
- `.github/prompts/*` use the structured module form (YAML frontmatter + Markdown sections), and the canonical `.copilot/skills/*` modules they route into use the same form.
- `.github/agents/*` may use either the `chatagent` discovery-wrapper form or the thin discoverability-card form depending on whether the file is a primary VS Code agent entrypoint or an alias stub.
- Use `templates/docs/ai-surface-template-checklist.md` when authoring or reviewing prompt/workflow surfaces, and update an accepted ADR first via `templates/docs/adr-template.md` when a wording change would alter precedence or canonical-form ownership.

## Shared checkpoints and constraints

- Prompt workflow entrypoints use repo-owned `.tmp/` state, never `/tmp`.
- `.tmp/github-issue-queue-state.md` is the shared checkpoint for queue progress, active issue/execution_lease_id/branch/worktree/PR state, validation evidence, and GitHub truth.
