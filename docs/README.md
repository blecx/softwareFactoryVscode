...
- [`maintainer/VALIDATION-POLICY-CONTRACT.md`](maintainer/VALIDATION-POLICY-CONTRACT.md) — canonical validation-policy authority note covering the official bundle taxonomy, per-bundle watchdog/runtime budgets, four-level model, changed-surface selection, explicit local-vs-GitHub exceptions, and later derivative migrations.
- [`maintainer/VALIDATION-RESOLVER-CONTRACT.md`](maintainer/VALIDATION-RESOLVER-CONTRACT.md) — shared validation plan resolver entrypoint, explaining deterministic changed-surface translation and downstream execution planning before runner execution.
- [`maintainer/VALIDATION-RUNNER-CONTRACT.md`](maintainer/VALIDATION-RUNNER-CONTRACT.md) — shared validation runner/reporting contract covering official bundle execution, per-bundle timing/status output, and the deferred caller-migration boundary for local/GitHub surfaces.
...
| [`PRODUCTION-READINESS-PLAN.md`](PRODUCTION-READINESS-PLAN.md)                                                       | Active supporting plan | Current readiness sequencing within the shipped `2.7` guardrails and the scope defined by `PRODUCTION-READINESS.md`. |
