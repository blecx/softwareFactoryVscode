...
**approved plan execution wrapper** — Manages a bounded, explicitly approved, GitHub-backed issue set end-to-end. This skill owns plan resolution, ordering, stop conditions, and repetition only. The single source of truth for issue-to-merge mechanics remains the canonical `resolve-issue` → `pr-merge` slice path.
...
2. If multiple plausible plans exist, stop and ask which plan or issue set is approved. Do not guess.
3. If the request is vague (`execute the plan`) and there is no unambiguous bounded issue set, ask a clarifying question before starting.
4. Never continue beyond the finite approved issue set named by the operator.
