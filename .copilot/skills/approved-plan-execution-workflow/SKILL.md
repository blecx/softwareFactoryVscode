...
- **Subagent No-Op Guard**: Validate each child subagent's result payload. A silent subagent no-op triggers exactly one targeted retry. If the retry also results in a no-op, escalate to a hard blocker. Forbids drifting to bypass or unbounded plan execution.
