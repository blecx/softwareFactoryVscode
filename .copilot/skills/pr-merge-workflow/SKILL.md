...
4. Confirm required CI/CD checks are green by explicitly polling:
   ...
   - If checks fail or the PR is not mergeable, do not invent a separate repair path. Hand the slice back to `resolve-issue` using the repository's default fast evidence-first repair method: parse the exact failed check/job/step output, quote the exact failing command/check, the relevant error text, and the suspected root cause, then use the **formatter-first** ladder — reproduce the cheapest failing gate first (touched-file formatter check: Black/isort on touched Python files when formatting drift is plausible) before single failing test, touched-test bundle, broader parity, or merge polling.
