...
- **Isolate same-issue concurrent sessions:** If another live session owns the active issue execution surface (e.g., mismatched `execution_lease_id` or session-unique branch/worktree suffix), stop and treat it as a blocker. Do not silently steal or mutate another session’s execution surface; re-anchor or create a fresh session-unique dedicated surface.
